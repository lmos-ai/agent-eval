...
class LLMEvaluation:
    ...
    def create_prompt(previous_context, query, actual_response, expected_response, actual_functions, expected_functions):
        ...
        def format_functions(functions):
            return "\n".join([
                f"Function: {function['function_name']} | Input parameters: {str(function.get('input_passed', {}))}" +
                (f" | Output parameters: {str(function.get('output_passed', {}))}" if 'output_passed' in function else "")
        ...
        Role:
        You are an expert evaluator for AI systems. Your role is to:
        ...
        - Provide detailed reasoning for whether they match or not.
            1.1 Thought process for correctness checking:
                - I have the latest response now i have to check if the expected response and latest function ouptut is given. If yes, then i need to check for the given latest user query if the latest response is correct by checking the expected response and function outputs. The expected response and function outputs are given  only to help me in deciding. I have to make decision checking previous context, and function outputs and expected response. If there is no expected response and latest function calling, then i have to check for the previous context and check for the user query if the latest response is correct or not. And if none of the previous context, function outputs and expected response is given, then use your knowledge and check if for the given user query wheter the response is correct or not.
...
def validate_conversation_logs(conversation_logs):
    ...
    """
    required_keys = ["user_query", "actual_function_calls", "response"]

    for idx, log in enumerate(conversation_logs):
        # Check if the log is a dictionary
        if not isinstance(log, dict):
            raise ValueError(f"Conversation log at index {idx} is not a dictionary: {log}")
        
        # Validate required keys
        missing_keys = [key for key in required_keys if key not in log]
        if missing_keys:
            raise ValueError(f"Missing keys in conversation log at index {idx}: {missing_keys}")
...
def validate_simulation_steps(simulation_steps):
    ...
    """
    required_keys = ["step_name", "trigger_event", "expected_response"]

    for idx, step in enumerate(simulation_steps):
        # Check if the step is a dictionary
        if not isinstance(step, dict):
            raise ValueError(f"Simulation step at index {idx} is not a dictionary: {step}")
